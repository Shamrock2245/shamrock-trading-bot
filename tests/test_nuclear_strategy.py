...
class TestNuclearTPLadder:
    ...
    def test_tp1_fires_at_5x(self):
        """TP1 should fire at 5x under nuclear profile."""
        pos = _make_position(entry_price=1.0)
        result = evaluate_position(pos, current_price=5.0, strategy_profile=NUCLEAR_PROFILE)
        assert result is not None, "Nuclear TP1 should fire at 5x"
...
class TestHardStops:
    ...
    def test_conservative_hard_stop_tolerance(self):
        ...
        result = evaluate_position(pos, current_price=0.85, strategy_profile=CONSERVATIVE_PROFILE)
        # Should NOT trigger hard stop (20% threshold)
        if result is not None:
            assert "stop" not in result.get("reason", "").lower(), (
                f"Conservative should NOT stop at -15%, got: {result.get('reason')}"
            )
...
class TestTPTierMarking:
    ...
    def test_nuclear_tp1_reason_format(self):
        """Nuclear TP1 reason should contain 'tp1_' (e.g. 'tp1_5x')."""
        pos = _make_position(entry_price=1.0)
        result = evaluate_position(pos, current_price=5.0, strategy_profile=NUCLEAR_PROFILE)
        if result:
            reason = result.get("reason", "")
            assert "tp1_" in reason, f"Expected 'tp1_' in reason, got: {reason}"
            # Verify the multiplier is in the reason (e.g. "tp1_5.0x")
