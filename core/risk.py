...
class RiskManager:
    ...
    def check_trade(
        ...
        Returns RiskCheck with approved=True and calculated position size,
        or approved=False with the blocking reason.
        """
        ...
        if wallet_balance_eth < settings.MIN_ETH_BALANCE_ALERT:
            return RiskCheck(
                approved=False,
                reason=(
                    f"ETH balance too low: {wallet_balance_eth:.4f} ETH "
                    f"(minimum: {settings.MIN_ETH_BALANCE_ALERT} ETH for gas)"
                ),
            )
        ...
        # Never spend more than 90% of balance (keep gas reserve)
